Introduction
============
With Lyo Designer, you can graphically model your vocabulary model according to the OSLC Core specification. A **Vocabulary Model** defines the types of classes, properties and their properties and relationships, according the [W3C Standard of RDF Vocabulary Description Language](https://www.w3.org/2001/sw/RDFCore/Schema/200203/).
...
A vocabulary model can also be imported into a toolchain model, to model a complete toolchain, as well as a single server and/or client. (see [Lyo Designer Manual](User-Manual-for-Toolchain-Modelling) for details).
...
You will need to create an Eclipse (modelling) project within which you will create the vocabulary model. In this model, you will potentially define a number of vocabulary models.

You may also choose to distribute the vocabulary models into a number of different Java libraries, each of which is itself an Eclipse project.
...
* each `vocabulary-project` contains the java classes corresponding to one or more vocavulary models, generated from the definitions in `oslc-model-project`.


Create a Vocabulary Model Modelling Project
------------------------------------------------
...
1.  Create a Vocabulary model
    1.  Right click the newly created project, and select **New &gt; other…**
    1.  In the Wizards window, navigate and select *Lyo Designer &gt; OSLC Vocabulary Model*, then press **Next**
    1.  choose a suitable file name (say *vocabulary.xml*) for the model, then press **Next**
    1.  Set **Model Object** to *Vocabularies*
    1.  Press **Finish**
    1.  Right-click the project again, and select **Viewpoints selection**
    1.  select **ToolChainViewpoint**
    1.  Press **OK**
1.  View the initial diagrams
    1. In the Model Explorer, by pressing the triangle/arrow to the left of the *vocavulary.xml* file , expand the file structure until the
        *VocabularyDiagram* entry is visible.
...
Model Vocabulary Diagram
===================

You are now ready to graphically specify the Vocabulary Model .
...
In the *VocabularyDiagram*, you can define a set of domain specifications, within which you define *Resources* and
*Resource Properties*.
...
1. Add the necessary maven dependencies by following the instructions under [Create an OSLC4J project](https://wiki.eclipse.org/Lyo/creating_OSLC4J_project).
